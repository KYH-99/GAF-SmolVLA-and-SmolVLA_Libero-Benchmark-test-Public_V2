...
def _infer_task_feature_source(critics, critic_checkpoints) -> str:
    ...
    task_feature_dim = getattr(critics[0].config, 'task_feature_dim', getattr(critics[0].config, 'language_dim', 0))
...
def main() -> None:
    ...
    if critic_paths:
        ...
        critic_task_feature_dim = getattr(critics[0].config, 'task_feature_dim', getattr(critics[0].config, 'language_dim', 0))
        if any(getattr(critic.config, 'task_feature_dim', getattr(critic.config, 'language_dim', 0)) != critic_task_feature_dim for critic in critics):
