...
def q_guided_velocity_smolvla_reverse_time(
    *,
    ...
    with torch.inference_mode(False), torch.enable_grad():
        ...
        if obs_features is not None: obs_features = obs_features.clone()
        if proprio is not None: proprio = proprio.clone()
        if task_features is not None: task_features = task_features.clone()
