...
class SmolVLAQGFProcessor:
    ...
    def set_batch_features(self, batch) -> None:
        self.obs_features = _latest_state(batch).clone()
        tf = _task_features_from_batch(
            batch,
            self.task_feature_dim,
            task_feature_source=self.task_feature_source,
            policy_model=self.policy_model,
        )
        self.task_features = tf.clone() if tf is not None else None
